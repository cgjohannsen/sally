 ...
 */
 ...
 term_t yices2_internal::mk_yices2_term(expr::term_op op, size_t n, term_t* children) {
   ...
   case expr::TERM_BV_SGT:
     ...
     break;
   case expr::TERM_BV_NEG:
     assert(n == 1);
     result = yices_bvneg(children[0]);
     break;
   case expr::TERM_BV_ROR:
     assert(n == 2);
     result = yices_rotate_right(children[0], children[1]);
     break;
   case expr::TERM_BV_ROL:
     assert(n == 2);
     result = yices_rotate_left(children[0], children[1]);
     break;
   default:
     assert(false);
   }
 ...
 public:
   ...
   void visit(expr::term_ref ref) {
     ...
     case expr::TERM_BV_SGN_EXTEND: {
       const expr::bitvector_sgn_extend& sign_extend = d_tm.get_bitvector_sgn_extend(t);
       term_t child = d_conversion_cache.get_term_cache(t[0]);
       result = yices_sign_extend(child, sign_extend.size);
       break;
     }
     default:
       assert(false);
